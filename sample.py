...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/finetune_addtoken.yaml",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to delta checkpoint of fine-tuned model block",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="coarse class descriptor of the target concept, like dog, cat",
    )
    parser.add_argument(
        "--replace_noun",
        type=str,
        default=None,
        help="auxiliary class descriptor of the target concept",
    )
    ...
    if opt.wandb_log:
        ...
        wandb.init(project="DCI_ICO", entity="cmu-gil", name=name )
    ...
    if opt.use_dci:
        ...
        modifier_token_str = opt.modifier_token.replace('+', " ")+(" ")
        prompt_ldm = opt.prompt.replace(modifier_token_str + opt.class_noun, opt.replace_noun)
        data_ldm = [batch_size * [prompt_ldm]]

        prompt_token = model.cond_stage_model.tokenizer(opt.prompt, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        ldm_prompt_token = model.cond_stage_model.tokenizer(prompt_ldm, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        class_token = model.cond_stage_model.tokenizer(opt.class_noun, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        replace_class_token = model.cond_stage_model.tokenizer(opt.replace_noun, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        
        length_custom = int(torch.nonzero(prompt_token[0] == 49407)[0] - 1)
        length_ldm = int(torch.nonzero(ldm_prompt_token[0] == 49407)[0] - 1)
        length_class_noun = torch.nonzero(class_token[0] == 49407)[0] - 1
        length_replace_noun = torch.nonzero(replace_class_token[0] == 49407)[0] - 1

        print('mm', length_class_noun)
        print('nn', length_replace_noun)

        custom_index_start = int(torch.nonzero(prompt_token[0] == 49408)[0][0])
        custom_index_end = int((custom_index_start + length_class_noun + len(list(opt.modifier_token.split('+'))))[0])
        print('11', custom_index_start)
        print('22', custom_index_end)
        print('33', length_class_noun)
        print('44', len(list(modifier_token_str.split('+'))))
        mask_custom = []
        for index in range(custom_index_start, custom_index_end):
            mask_custom.append(index-1)
        
        ldm_index_start = torch.nonzero(ldm_prompt_token[0] == int(replace_class_token[0][1]))[0]
        ldm_index_end = ldm_index_start + length_replace_noun
        print('aa', ldm_index_start)
        print('bb', ldm_index_end)

        mask_ldm = []
        for index in range(1, length_ldm+1):
            if index< ldm_index_start or index>=ldm_index_end:
                mask_ldm.append(index-1)
        
        print('11', length_custom)
        print('22', length_ldm)
        print('33', mask_custom)
        print('44', mask_ldm)
        print('55', opt.prompt)
        print('66', prompt_ldm)
        ...
        print('ldm_prompt_token', ldm_prompt_token)
        print('class_token', class_token)
        print('replace_class_token', replace_class_token)
        #if not opt.from_file:

        extra_info = {'length_ldm': length_ldm, 'length_custom': length_custom, 'mask_ldm': mask_ldm, 'mask_custom': mask_custom, 'share_guidance': opt.share_guidance, 'agg_time': opt.agg_time, 'lambda': opt.agg_lambda}
    else:
        extra_info = None
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                prompt_count += 1
                p = str(prompts[0]).replace('<new1> ', '').replace('<new2> ', '').replace('<new3> ', '')
                print(p)
                ref_image_paths = glob.glob("data/tortoise_plushy/*")
                clip_score_cal(images_paths, ref_image_paths, p, device = device)
